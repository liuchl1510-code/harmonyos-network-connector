...
应用显示名称为 **Harmony VPN**，当前开发版本为 **0.26.0/code48**。这是独立的开发项目，参考 v2rayNG 的功能与分享链接格式，不是 v2rayNG 或华为的官方客户端。完整 Android 功能对等和商店发布尚未完成。

0.26.0 在 Go 核心的同一 DNS 实例内复用等价的远端 DoH HTTP2 连接，保留各解析器独立的缓存、路由标签与失败策略。手机分流模式第二类别首次 DNS 查询的中位数为 221／225.5 毫秒，且没有新增 DoH 拨号；历史对应值为 742.5／712 毫秒。两个批次并非紧邻随机 A/B，冷启动 HTTPS 未显示稳定改善。21 个本地叶子场景、三轮竞态检查及 42 个策略场景通过；手机 16 次测量会话与一次日常连接复验均正常清理，7 份用户文件按字节保留。没有切换物理网络，C++ 和 Hev 源码未修改，详见[阶段三十三记录](HarmonyVpnLab/docs/phase33-doh-pool.md)。

0.25.1 增加仅调试包可用的受控 HTTPS 连接复用诊断：两个请求各用独立 SDK 对象，记录实际复用标志，并在统一清理后保存结果。一台 API26／ARM64 手机完成 12 次基线会话和 4 次连接复用会话，共 48 项测量；首次 HTTPS 约 1.6 秒，确认复用后的请求约 190 毫秒。DNS 交叉对照表明分流类别的独立 DoH 连接存在额外首次连接开销；本版本没有修改原生转发核心，不能据此声称日常转发性能已提高。测试未切换物理网络，结束后 VPN 断开，7 份用户文件按字节一致，详见[阶段三十二记录](HarmonyVpnLab/docs/phase32-first-access.md)。

0.25.0 完善 DNS 检查与故障验证：白名单 DNS 分流连接分别检查直连与经节点查询，直连失败后仍可显示代理 DNS／HTTPS 的结果；旧检查在网络恢复或会话变化后失效。新增仅调试包可用、只接管本应用的受控故障入口，临时策略只在内存使用，不改用户设置。一台 API26／ARM64 手机已完成升级、正常分流检查、真实 HTTP 404 与极短超时故障检查，并核对故障会话 90 秒自动结束；同会话断网自动恢复、离线等待时主动停止及恢复后的不重连观察已核对，见[阶段三十一记录](HarmonyVpnLab/docs/phase31-dns-recovery.md)。

0.24.0 增加默认关闭的 **白名单 DNS 分流**：已知直连域名可通过当前物理网络查询直连 DoH，Google 类别优先经节点，其余域名继续经节点。默认保留全部经节点查询；全局和自定义规则模式不启用 DNS 分流。直连 DoH 默认使用阿里公共 DNS（223.5.5.5），提供方可见查询域名及物理网络出口；失败时不跨 DNS 路径回退。一台鸿蒙电脑已完成 18 次连接对照、30 项网络测量及重连检查；百度改善明显，Cloudflare 未改善，见[阶段三十记录](HarmonyVpnLab/docs/phase30-split-dns.md)。

0.23.0 增加 **白名单（绕过大陆）**：按 v2rayN 7.24.9 内置规则及固定分类数据分流，保留例外顺序，未命中走代理。默认仍为全部代理；入口为首页“网络设置”。鸿蒙电脑已通过全局 → 白名单 → 全局实测，见[白名单实现与验证](HarmonyVpnLab/docs/phase28-whitelist-routing.md)。DNS 继续经节点发送，IPv6 仍阻断。

0.22.0 增加诊断摘要：从连接诊断进入预览，可复制到本机剪贴板或保存 TXT，并核对文件内容。摘要只整理固定分类、时间和计数，排除节点地址、凭据与原始错误；明确区分空记录、格式异常和读取失败。实现与验证范围见[阶段二十六记录](HarmonyVpnLab/docs/phase26-diagnostic-summary.md)。

2026-09-22 已在一台 API 26、ARM64 鸿蒙电脑上完成完整核心的短时实测：两次连接、三次代理 DNS／HTTPS 检查及两次断开清理通过，并检查宽窄窗口、大字号和诊断摘要导出，详见[真实电脑验证记录](HarmonyVpnLab/docs/phase27-real-pc.md)。0.23.0 随后完成约 30 分钟后台、系统睡眠唤醒、断网恢复与主动断开后的不重连观察，详见[阶段二十九记录](HarmonyVpnLab/docs/phase29-pc-stability.md)；这些历史结果不替代新 DNS 分流的验收。

0.21.0 改用悬浮玻璃胶囊导航，右侧圆形按钮可快捷连接与断开；页面内容延伸到导航后方，深浅色边缘与多端布局同步调整。实现与验证范围见[阶段二十五记录](HarmonyVpnLab/docs/phase25-floating-navigation.md)。

0.20.0 增加按应用代理范围：全部应用、绕过所选应用、仅代理所选应用。从常见应用预设中勾选，其他应用可按包名添加；安装状态由用户确认。应用范围独立于域名/IP 分流，保存后下次连接生效。实现与当前验证状态见[阶段二十四记录](HarmonyVpnLab/docs/phase24-app-routing.md)。
...
各版本变化与验证边界见[完整变更日志](CHANGELOG.md)。0.26.0 修改 Go 核心的 DNS 传输管理；各版本的历史验收不能替代当前版本结果。
...
- 全部代理、内置 v2rayN 白名单或自定义域名/IPv4 分流、绕过局域网、可配置的 HTTPS DNS；白名单 DNS 分流为默认关闭的可选模式。
- 按应用设置代理范围，提供常见鸿蒙应用预设并支持按包名添加；客户端自身固定纳入以进行连接检查。
- 网络变化后的恢复、等待网络时正常断开。
- 有界诊断记录与连接状态显示；可预览、复制和导出不含节点凭据的诊断摘要。
- 导入、编辑及连接前的本地配置检查；首页可手动检查当前节点，失败提示包含字段、问题和建议。
- 首页与诊断页区分 VPN 启动、节点地址解析、核心初始化、直连 DNS、代理 DNS、HTTPS 等失败阶段，不根据原始错误文本猜测根因。分流模式分别显示两条 DNS 检查结果，DNS 失败不直接认定节点失败。

IPv6 当前进入 VPN 后被阻断，尚未提供 IPv6 代理。节点凭据保存在应用私有目录，尚未增加应用层配置加密。应用不提供节点或订阅服务；测试和实际使用需要自行配置。

**多端界面适配不等于多端 VPN 已通过。** 当前 x86_64 模拟器包仅用于界面预览，不包含 Xray/Hev 转发核心，连接、检测和开发验证均被禁用。真实平板安装与联网按用户要求暂停，尚未验收；一台 API26、ARM64 鸿蒙电脑的限定联网与恢复场景已验收，不能外推为全部电脑架构、系统版本或新功能均已通过。

## 界面

以下为 **0.21.0 x86_64 界面预览包**的悬浮玻璃导航。胶囊后方可以透出滚动内容，右侧为独立启停按钮；预览包不含 VPN 核心，因此按钮禁用。深浅色、系统区边缘及多端验证见[阶段二十五记录](HarmonyVpnLab/docs/phase25-floating-navigation.md)。

| 浅色 | 深色 |
| --- | --- |
| <img src="docs/images/0.21-phone-glass-light.jpeg" alt="浅色手机界面的悬浮玻璃导航和独立圆形按钮" width="280" /> | <img src="docs/images/0.21-phone-glass-dark.jpeg" alt="深色手机界面的悬浮玻璃导航，内容延伸至胶囊后方" width="280" /> |
