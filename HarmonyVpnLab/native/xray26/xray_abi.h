...
// Reads current nodeProxy counters without HTTP/expvar; serialized with Run/Stop.
// data: {"uplink":number,"downlink":number}. A stopped core or absent counters
// returns success:false and a fixed error. CGoQueryStats keeps its old behavior.
char *CGoConnectionStats(void);
