// This executable validates synthetic connection fixtures only. It never creates
...
type result struct {
	...
	ProtoChecked bool `json:"protoChecked"`
}
...
func validate(sample fixture) (out result) {
	...
	if failure := checkConnectionProto(config); failure != "" {
		out.Failure = failure
		return
	}
	out.ProtoChecked = true
	out.Passed = true
	return
}
...
func run() int {
	...
	verification := report{
		...
		ValidationScope: "synthetic JSON -> core.LoadConfig -> explicit DNS/routing/policy protobuf assertions",
