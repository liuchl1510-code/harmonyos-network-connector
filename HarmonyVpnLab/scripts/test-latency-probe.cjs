...
function scenario(options = {}) {
  ...
  const calls = { create: 0, request: [], destroy: 0, fs: [] };
  ...
  const shared = { fs: fakeFs, Date: FakeDate,
    ...
    setTimeout(fn, ms) { const id = nextTimer++; timers.set(id, { fn, ms, repeat: false }); return id; },
    ...
    http: { RequestMethod: { GET: 0 }, HttpDataType: { STRING: 1 }, createHttp() {
      calls.create++; fail('create'); return {
        request(url, opts) { calls.request.push({ url, opts }); fail('request'); return pending.promise; },
        destroy() { calls.destroy++; fail('destroy'); }
      };
    } }
  };
  ...
  return { files, handles, timers, clock, faults, calls, pending, protocol, probe, request,
...
test('one complete HTTPS response measures monotonic request duration only', async () => {
  ...
  assert.deepEqual(Object.keys(proof).sort(), ['runId', 'nodeId', 'outboundFingerprint', 'status', 'durationMs', 'reason', 'checkedAt'].sort());
...
test('request uses fixed HTTPS destination, secure TLS defaults, no redirects or caching and bounded body', async () => {
  ...
  s.pending.resolve(goodResponse()); await pending; cleaned(s);
});
...
test('caller mutation cannot relabel a response for a different node or run', async () => {
  ...
  s.pending.resolve(goodResponse()); const proof = await pending;
...
test('native HTTP errors become fixed https results without body or raw error text', async () => {
  ...
  for (const fail of ['request', 'destroy', 'clock']) {
    ...
    if (fail === 'destroy') fault.pending.resolve(goodResponse());
