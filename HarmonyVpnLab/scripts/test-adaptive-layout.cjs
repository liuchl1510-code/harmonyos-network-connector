...
const sourceSHA256={},hash=value=>crypto.createHash('sha256').update(value).digest('hex');
function load(name,imports={}){const file=path.join(root,'entry/src/main/ets',name),source=fs.readFileSync(file,'utf8');sourceSHA256[name]=hash(source);const result=ts.transpileModule(source,{compilerOptions:{target:ts.ScriptTarget.ES2021,module:ts.ModuleKind.CommonJS},reportDiagnostics:true});assert.equal(result.diagnostics.length,0);const m=new Module(file);m.require=k=>{assert(Object.hasOwn(imports,k),k);return imports[k]};m._compile(result.outputText,file);return m.exports;}
const layout=load('model/AdaptiveLayout.ets');let passed=0;const results=[];function test(name,fn){fn();passed++;results.push({name,passed:true});console.log('PASS '+name)}
...
test('window resize updates density-aware vp and removes exact listener on destroy',()=>{
  const values=new Map(),events=[],main={getUIContext:()=>({px2vp:n=>n/2}),getWindowProperties:()=>({windowRect:{width:2560,height:1600}}),getWindowAvoidArea:type=>{assert.equal(type,0);return{topRect:{height:64}}},on:(name,fn)=>events.push({name,fn}),off:(name,fn)=>{assert.equal(name,'windowSizeChange');assert.equal(fn,events[0].fn);events.push({off:true})}};
  global.AppStorage={setOrCreate:(k,v)=>values.set(k,v)};
  const {default:Ability}=load('entryability/EntryAbility.ets',{'@kit.AbilityKit':{UIAbility:class{}},'@kit.ArkUI':{window:{AvoidAreaType:{TYPE_SYSTEM:0}}},'@kit.BasicServicesKit':{deviceInfo:{deviceType:'phone'}},'@kit.PerformanceAnalysisKit':{hilog:{info(){},warn(){}}},'libvpnbridge.so':{},'../model/Appearance':{applyAppearance(){},readAppearance:()=> 'system'},'../model/AdaptiveLayout':layout,'../model/BuildCapabilities':{VPN_CORE_AVAILABLE:true}});
  const a=new Ability();a.mainWindow=main;a.attachWindowSize();assert.equal(values.get('windowWidthVp'),1280);assert.equal(values.get('statusBarHeightVp'),32);
  ...
  a.attachWindowSize();assert.equal(events.length,1);a.onWindowStageDestroy();assert.equal(events.length,2);assert.equal(a.mainWindow,undefined);assert.equal(values.get('statusBarHeightVp'),0);
});
const out=path.join(root,'build/adaptive-layout-verification.json');fs.mkdirSync(path.dirname(out),{recursive:true});fs.writeFileSync(out,JSON.stringify({generatedAt:new Date().toISOString(),passed,failed:0,total:results.length,results,sourceSHA256,testFileSha256:hash(fs.readFileSync(__filename)),scope:'actual pure layout and EntryAbility resize listener with synthetic SDK; status inset is provided by the window adapter; not visual emulator evidence'},null,2));console.log(JSON.stringify({passed,failed:0}));
