...
**当前 0.16.0/code37 增加首次 HTTPS 与复用连接双指标。** 两次请求分别计时，只有系统明确确认连接复用才显示“复用延迟”；旧记录、第二次失败和未确认复用均有独立提示。排序仍按首次耗时。9 月 11 日初版见[阶段十八记录](docs/phase18-latency-metrics.md)，9 月 12 日两页修正后的当前包与真机复验见[收尾记录](docs/phase20-restoration-and-copy.md)。
...
当前版本：0.16.0/code37。完整核心构建继续使用 Go 1.26.7 的 OHOS 适配和真实 Xray 26.6.1，满足服务端默认最低 26.3.27 的要求。逐 socket 保护、运行时时钟和保存读回校验继续保留。0.8.1 用第9个包装层ABI直接读取当前核心统计，移除了不支持重复启动的HTTP metrics模块。0.6.4 修复授权观察器回收卡死并通过一次锁屏恢复；0.6.8 修复 Hev 可写事件空转。历史CPU采样不是续航测量，版本/测试范围分别记录。
...
预览版不需要准备 ARM64 库，0.16.0 构建的版本标记为 `0.16.0-ui-preview`，产物写入 `build/artifacts/simulator-ui/`。连接、节点联网检测和开发验证入口均不可用；节点编辑、粘贴导入、备份恢复和外观仍可检查。安装到模拟器需自己的签名配置，命令见[预览构建指南](docs/public-build.md#4-x86_64-模拟器界面预览)。当前脚本的构建模式仍是 debug，本轮状态以[阶段十八记录](docs/phase18-latency-metrics.md)为准。
...
节点列表的“检测”分别记录两次完整 HTTPS 请求耗时，可在断开状态执行。首次值包含本次 DNS、连接及响应，不含 VPN 启动时间；随后同一请求对象再次访问同一地址，确认复用才显示“复用延迟”。第二次失败保留首次成功并单独提示，旧记录标明尚未检测复用，排序使用首次值。固定访问 Cloudflare trace，TLS 正常验证、禁用跳转和响应缓存，每请求最多 8 秒、两请求最多 16 秒，临时会话上限 30 秒。网络变化或取消使迟到结果失效；不切换当前节点。每节点只保留最近一次有效格式的记录，配置改变后隐藏旧结果；这些耗时不是 TCP RTT 或带宽。
