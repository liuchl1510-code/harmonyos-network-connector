...
**当前 0.15.0/code36 增加节点收藏、来源筛选与多端详情。** 导入成功后可直接查看本次结果，收藏随 v2 备份保存，旧版数据兼容读取。ARM64 与 x86_64 调试签名构建已核验；API 26 真机完成从 0.11.1 升级、原节点与设置保留、收藏恢复和一次短时 Wi-Fi DNS/HTTPS 检查，结束后确认断开及清理。实际操作与验证范围见[阶段十六记录](docs/phase16-node-collection.md)。
...
当前版本：0.15.0/code36。完整核心构建继续使用 Go 1.26.7 的 OHOS 适配和真实 Xray 26.6.1，满足服务端默认最低 26.3.27 的要求。逐 socket 保护、运行时时钟和保存读回校验继续保留。0.8.1 用第9个包装层ABI直接读取当前核心统计，移除了不支持重复启动的HTTP metrics模块。0.6.4 修复授权观察器回收卡死并通过一次锁屏恢复；0.6.8 修复 Hev 可写事件空转。历史CPU采样不是续航测量，版本/测试范围分别记录。
...
预览版不需要准备 ARM64 库，0.15.0 构建的版本标记为 `0.15.0-ui-preview`，产物写入 `build/artifacts/simulator-ui/`。连接、节点联网检测和开发验证入口均不可用；节点编辑、粘贴导入、备份恢复和外观仍可检查。安装到模拟器需自己的签名配置，命令见[预览构建指南](docs/public-build.md#4-x86_64-模拟器界面预览)。当前脚本的构建模式仍是 debug，本轮预览验收状态以[阶段十六记录](docs/phase16-node-collection.md)为准。
