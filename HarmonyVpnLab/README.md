...
**当前 0.14.0/code35 完善表单数据保护和列表响应。** 节点导入、订阅及网络设置增加未保存返回确认，失败保留输入，提交后的读回异常单独反馈；修复编辑页取消返回后草稿回退和窄窗口大字号按钮横向溢出。两种调试构建已核验，模拟器完成 25 步实际表单操作、500 节点合成排序对照及平板 20 分钟界面观察；手机 60 分钟观察本轮未完成，仅保留约 20 分钟的部分样本。本轮未修改原生转发核心，真实设备安装与 VPN 联网继续暂停。使用与证据见[阶段十四记录](docs/phase14-autonomous-refinement.md)。
...
当前版本：0.14.0/code35。完整核心构建继续使用 Go 1.26.7 的 OHOS 适配和真实 Xray 26.6.1，满足服务端默认最低 26.3.27 的要求。逐 socket 保护、运行时时钟和保存读回校验继续保留。0.8.1 用第9个包装层ABI直接读取当前核心统计，移除了不支持重复启动的HTTP metrics模块。0.6.4 修复授权观察器回收卡死并通过一次锁屏恢复；0.6.8 修复 Hev 可写事件空转。历史CPU采样不是续航测量，版本/测试范围分别记录。
...
预览版不需要准备 ARM64 库，0.14.0 构建的版本标记为 `0.14.0-ui-preview`，产物写入 `build/artifacts/simulator-ui/`。连接、节点联网检测和开发验证入口均不可用；节点编辑、粘贴导入、备份恢复和外观仍可检查。安装到模拟器需自己的签名配置，命令见[预览构建指南](docs/public-build.md#4-x86_64-模拟器界面预览)。当前脚本的构建模式仍是 debug，本轮预览验收状态以[阶段十四记录](docs/phase14-autonomous-refinement.md)为准。
