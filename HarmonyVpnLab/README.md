...
**当前开发版本为 0.26.0/code48：复用等价的 DoH HTTP2 连接。** 同一 Go DNS 实例内，普通远端 HTTPS 的完整 URL 与有效标签相同才共池，保留独立缓存和查询策略，并管理取消、握手与关闭。手机分流第二类别首次 DNS 中位数为 221／225.5 毫秒，无新增 DoH 拨号；冷启动 HTTPS 未显示稳定改善。17 个测量／日常连接会话均清理，7 份用户文件按字节保留，最后 VPN 断开；未切换物理网络，C++／Hev 源码未修改，详见[阶段三十三记录](docs/phase33-doh-pool.md)。

0.25.1 增加受控 HTTPS 连接复用诊断。仅用于完整核心调试包，两个 GET 各用独立 SDK 对象，实际复用由 SDK 确认，统一释放后写入收据。手机自动对照完成 16 次会话、48 项测量；首次请求约 1.6 秒，确认复用后约 190 毫秒。测试未切换物理网络，用户文件按字节保留，最后 VPN 断开。该版本没有修改原生转发核心，详见[阶段三十二记录](docs/phase32-first-access.md)。

0.25.0 完善 DNS 检查与受控故障验证。白名单 DNS 分流分别显示直连、经节点 DNS 与 HTTPS 结果；新增仅用于调试包的本应用故障验证，临时策略不写回用户设置。API26／ARM64 手机的升级、正常分流、HTTP 404 与极短超时故障及正常／自动结束清理已核对；同会话断网自动恢复、离线等待时主动停止及恢复后的不重连观察已核对，详见[阶段三十一记录](docs/phase31-dns-recovery.md)。

0.24.0 新增默认关闭的白名单 DNS 分流。该选项仅在白名单模式下生效；原代理 DoH 与直连 DoH 分别保存，旧网络设置仅在内存迁移，显式保存后写回 schema 3。一台鸿蒙电脑已完成 18 次连接对照及重连检查；不同网站结果不一致，使用方法与边界见[阶段三十记录](docs/phase30-split-dns.md)。

0.23.0 新增 v2rayN 白名单（绕过大陆）。网络设置中可选择内置白名单；规则顺序、分类资源和实际电脑分流测试见[阶段二十八记录](docs/phase28-whitelist-routing.md)。默认仍为全部代理，既有自定义列表与应用范围保留。该版本随后完成约 30 分钟后台与睡眠、网络恢复验证，见[阶段二十九记录](docs/phase29-pc-stability.md)。

0.22.0 增加诊断摘要预览、复制与 TXT 导出，区分缺失、空记录、损坏和读取失败；复制与保存使用同一份预览，TXT 写入后按字节核对。见[阶段二十六记录](docs/phase26-diagnostic-summary.md)。

该版本已在一台 API 26、ARM64 真实鸿蒙电脑上完成安装、原生运行时、宽窄窗口控件、两次 VPN 连接与清理以及摘要导出的短时检查。详细证据和未覆盖范围见[阶段二十七记录](docs/phase27-real-pc.md)。

0.21.0 增加悬浮玻璃导航与独立快捷启停。 底部透明叠放在页面内容上，支持系统沉浸材质及旧版本半透明模糊降级；宽窗口保留侧导航。见[阶段二十五记录](docs/phase25-floating-navigation.md)。

0.20.0 增加按应用设置代理范围。支持全部应用、绕过所选应用、仅代理所选应用；使用常见应用预设或手工包名，安装状态由用户确认，保存后下次连接生效。应用范围独立于原有分流与 DNS，客户端自身固定纳入连接检查。实现与验证状态见[阶段二十四记录](docs/phase24-app-routing.md)。

**0.15.0 增加节点收藏、来源筛选与多端详情。** 导入成功后可直接查看本次结果，收藏随 v2 备份保存，旧版数据兼容读取。该版本的构建、升级、收藏恢复及 Wi-Fi DNS/HTTPS 结果见[阶段十六记录](docs/phase16-node-collection.md)，不替代当前版本的验证。

这是鸿蒙原生 VPN 移植的验证工程。当前已集成 ArkTS 界面、C++ 桥接、Hev 2.9.0 和 Xray 26.6.1。

**历史 0.14.0/code35 完善表单数据保护和列表响应。** 节点导入、订阅及网络设置增加未保存返回确认，失败保留输入，提交后的读回异常单独反馈；修复编辑页取消返回后草稿回退和窄窗口大字号按钮横向溢出。两种调试构建已核验，模拟器完成 25 步实际表单操作、500 节点合成排序对照及平板 20 分钟界面观察；手机 60 分钟观察本轮未完成，仅保留约 20 分钟的部分样本。本轮未修改原生转发核心，真实设备安装与 VPN 联网继续暂停。使用与证据见[阶段十四记录](docs/phase14-autonomous-refinement.md)。

**历史 0.13.0/code34 细化首页、节点列表、设置与编辑体验。** 首页首次使用可直接添加节点，网络设置可直达，宽屏增加配置区；节点列表采用紧凑行、当前标记与筛选数量，检测说明默认折叠；设置分为“连接与数据”和“帮助与说明”，外观选项保留选中勾选并减少重复提示；节点编辑增加未保存离开保护、固定反馈区及端口数字输入与校验。API 24 兼容代码已补充版本/能力判断、启动超时和请求代次隔离。该轮签名构建与有界模拟器页面观察已完成，但当时编辑确认和非法端口完整 GUI 流程未完成验收，仅有离线验证。历史实现与证据见[阶段十三记录](docs/phase13-product-refinement.md)。
...
当前版本：0.26.0/code48。完整核心构建继续使用 Go 1.26.7 的 OHOS 适配和 Xray 26.6.1，并增加本地 DoH 传输补丁；核心版本常量保持，满足服务端默认最低 26.3.27 的要求。逐 socket 保护、运行时时钟和保存读回校验继续保留。0.8.1 用第9个包装层ABI直接读取当前核心统计，移除了不支持重复启动的HTTP metrics模块。0.6.4 修复授权观察器回收卡死并通过一次锁屏恢复；0.6.8 修复 Hev 可写事件空转。历史CPU采样不是续航测量，版本/测试范围分别记录。
...
预览版不需要准备 ARM64 库；按当前版本构建时，版本标记为 `0.26.0-ui-preview`，产物写入 `build/artifacts/simulator-ui/`。连接、节点联网检测和开发验证入口均不可用；节点编辑、粘贴导入、备份恢复和外观仍可检查。安装到模拟器需自己的签名配置，命令见[预览构建指南](docs/public-build.md#4-x86_64-模拟器界面预览)。当前脚本的构建模式仍是 debug；此处描述构建规则，不代表本轮已产出或验收预览包。
...
DNS 默认全部经节点查询，原代理 DoH 默认仍为 Cloudflare（1.1.1.1）。断开连接后，可在首页“网络设置”选择白名单及“白名单 DNS 分流”，保存后下次连接生效：Google 类别优先经节点，内置大陆、私有及公共 DNS 服务域名类别走直连 DoH，其余域名经节点。直连 DoH 默认阿里公共 DNS（223.5.5.5），仅支持 HTTPS IPv4 地址；提供方可见查询域名和物理网络出口，公共服务不保证解析局域网私有域名。查询失败不跨 DNS 路径回退，按 IP 分流的连接也不一定使用直连 DNS。全局／自定义规则模式保留该偏好但不生效，节点检测仍使用独立默认配置。

安装后首页是持续连接页。已保存节点时，点击“连接”按网络设置中保存的应用范围启动 IPv4 代理（默认全部应用），点击“检查连接”验证本应用的代理 DNS 与域名 HTTPS，点击“断开”释放连接。“90 秒全设备验证”和“90 秒本应用验证”会自动结束，普通“连接”没有此计时器。进入 VPN 的 IPv6 由核心黑洞处理，未提供 IPv6 代理；绕过应用的 IPv6 不由本客户端阻断。
...
节点列表的“检测”分别记录两次完整 HTTPS 请求耗时，可在断开状态执行。首次值包含本次 DNS、连接及响应，不含 VPN 启动时间；随后同一请求对象再次访问同一地址，确认复用才显示“复用延迟”。第二次失败保留首次成功并单独提示，旧记录标明尚未检测复用；可恢复原始顺序，也可按首次值或已确认复用延迟排序。固定访问 Cloudflare trace，TLS 正常验证、禁用跳转和响应缓存，每请求最多 8 秒、两请求最多 16 秒，临时会话上限 30 秒。网络变化或取消使迟到结果失效；不切换当前节点。每节点只保留最近一次有效格式的记录，配置改变后隐藏旧结果；这些耗时不是 TCP RTT 或带宽。

“设置”中的“连接诊断”在连接中也可以查看，最多保留最近200条服务事件及50条应用观察；服务事件是固定类别事件：网络变化、恢复、保护失败分类和停止清理。Socket热路径只累计计数，心跳和停止时写入分类记录；节点、地址和原始核心日志不写进诊断页。
