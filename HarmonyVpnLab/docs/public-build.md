...
Git 仓库只提供源代码、固定版本记录、补丁和许可证文本。`entry/libs/arm64-v8a/*.so`、HAP、下载的上游源码、编译器缓存和个人签名资料均不随 Git 发布。当前开发版本为 0.23.0/code44，构建与验证状态见[阶段二十八记录](phase28-whitelist-routing.md)；公开构建配置的最低兼容版本为 `6.1.1(24)`，compile/target SDK 仍为 `26.0.0`。API 24 真机安装与联网已按用户要求暂停，兼容配置不等于验收通过。先按目标选择构建变体：
...
验收顺序是先完成模拟器 UI；**真实 API24 平板的安装与 VPN 联网验证当前按用户要求暂停**，恢复验收时还需单独核对系统与签名条件。0.22.0 已在一台 API26、ARM64 真实鸿蒙电脑上完成两次连接、代理 DNS／HTTPS 与断开清理的短时验收，见[阶段二十七记录](phase27-real-pc.md)。新设备需要加入开发者自己的调试签名。当前 Go amd64 TLS/运行时限制只对应 x86_64 核心，不能泛化为所有 PC；也不能把这台 ARM64 电脑的结果外推为 x86_64 联网通过。电脑睡眠／唤醒、全天稳定性、续航及完整折叠/旋转链路仍需单独验证。
