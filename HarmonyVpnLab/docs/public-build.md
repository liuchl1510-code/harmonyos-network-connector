...
Git 仓库只提供源代码、固定版本记录、补丁和许可证文本。`entry/libs/arm64-v8a/*.so`、HAP、下载的上游源码、编译器缓存和个人签名资料均不随 Git 发布。当前开发版本为 0.26.0/code48，标准 Xray 构建会应用实例内 DoH 共池补丁并执行本地 HTTP2／生命周期检查；补丁、产物和手机结果见[阶段三十三记录](phase33-doh-pool.md)。0.25.1 的手机自动性能对照见[阶段三十二记录](phase32-first-access.md)；0.25.0 的 DNS 故障与恢复检查见[阶段三十一记录](phase31-dns-recovery.md)，0.24.0 的鸿蒙电脑 DNS 对照单独保留在[阶段三十记录](phase30-split-dns.md)。公开构建配置的最低兼容版本为 `6.1.1(24)`，compile/target SDK 仍为 `26.0.0`。API 24 真机安装与联网已按用户要求暂停，兼容配置不等于验收通过。先按目标选择构建变体：
...
验收顺序是先完成模拟器 UI；**真实 API24 平板的安装与 VPN 联网验证当前按用户要求暂停**，恢复验收时还需单独核对系统与签名条件。0.22.0 已在一台 API26、ARM64 真实鸿蒙电脑上完成两次连接、代理 DNS／HTTPS 与断开清理的短时验收，见[阶段二十七记录](phase27-real-pc.md)；0.23.0 的约 30 分钟后台、系统睡眠唤醒及断网恢复见[阶段二十九记录](phase29-pc-stability.md)。新设备需要加入开发者自己的调试签名。当前 Go amd64 TLS/运行时限制只对应 x86_64 核心，不能泛化为所有 PC；也不能把这台 ARM64 电脑的历史结果外推为 x86_64 联网或新 DNS 分流通过。全天稳定性、续航及完整折叠/旋转链路仍需单独验证。
