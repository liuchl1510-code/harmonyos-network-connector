...
Git 仓库只提供源代码、固定版本记录、补丁和许可证文本。`entry/libs/arm64-v8a/*.so`、HAP、下载的上游源码、编译器缓存和个人签名资料均不随 Git 发布。当前开发版本为 0.18.0/code39，构建与验证状态见[阶段二十二记录](phase22-session-recovery.md)；公开构建配置的最低兼容版本为 `6.1.1(24)`，compile/target SDK 仍为 `26.0.0`。API 24 真机安装与联网已按用户要求暂停，兼容配置不等于验收通过。先按目标选择构建变体：
